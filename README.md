...
![Version](https://img.shields.io/badge/Version-2.2.4-brightgreen.svg)
...
### 4. Метки терапии и заметок на графике (Treatments & Notes Overlay)
...
### 5. Линия прогноза тренда на 25 минут (Trend Forecast Overlay)
...
### 7. Защита близких: Роли «Мастер / Фоловер», Heads-Up сообщения и экстренный SOS
...
### 8. Плавающий оверлей «Пузырёк» (Floating Bubble)
...
### 10. Воскресный аналитический дайджест (Sunday Digest)
...
### 11. 24-часовой амбулаторный профиль глюкозы (AGP) и паттерны
...
### 12. Медицинские PDF-отчёты AGP и Руководство пользователя
...
- **Автоматическая синхронизация и продление по заметкам из xDrip+**:
  ...
  - Поддержка указания дней (`рестарт канюли 3`, `продлить сенсор на 7 дней`, `ланцет +7`, `extend cannula 4`): сбрасывает дату установки на время заметки и обновляет срок цикла.

### 20. Энергоэффективный Always-On Display (AoD) и ночные прикроватные часы
...
- **Гигантский сахар и тренд в ландшафтном режиме**:
  - Адаптивный крупный шрифт значения сахара и стрелки тренда для комфортного чтения с прикроватной тумбочки ночью.
  - Крупная 5-минутная дельта и возраст замера с поджатием к цифрам сахара.
  - Верхняя панель с ночными часами, индикатором батареи (`🔋 XX%` / `⚡ XX%`) и кнопкой закрытия.
- **Интерактивные жесты и управление**:
  - **Вертикальный свайп по экрану**: плавная непрерывная регулировка яркости экрана без отрыва пальца с информационным HUD вверху.
  - **Двойной тап**: режим ночного тёплого фонарика с плавным 15-секундным разгоранием до 100%, паузой по одинарному тапу и интерактивным слайдером ручной подстройки.
...
- **Автозапуск**: опциональный автоматический запуск при постановке смартфона на ночную зарядку.
