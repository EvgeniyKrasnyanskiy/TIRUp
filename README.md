...
- **Офлайн SMS-запрос сахара без интернета**:
  ...
  - *Примечание по Android*: если приложение не отвечает на входящие SMS, в настройках разрешений Android переключите доступ к SMS в «Запретить» и обратно в «Разрешить» для сброса кэша прав ОС.
...
4. **Автоматический учёт канюли помпы**: при сохранении в xDrip+ записи с комментарием `«канюля»`, `«инфуз»` или `«cannula»`, TIRUp автоматически синхронизирует и обновляет дату установки инфузионного набора.
